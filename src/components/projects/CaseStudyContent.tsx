...
function ClickableImage({
  ...
  return (
    ...
    >
      <ProjectImage src={src} alt={alt} sizes={sizes} priority={priority} />
      <span
        aria-hidden="true"
...
function MetaItem({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div>
      ...
      <span className="block font-body text-[15px] leading-snug text-paper">{children}</span>
    </div>
  );
}
...
export function CaseStudyContent({ project }: { project: Project }) {
  ...
  const images: LightboxImage[] = [
    ...
    { src: project.solutionImage, alt: `${project.title} — solution` },
    ...
    <article>
      ...
      <motion.header initial="hidden" animate="visible" variants={heroFade}>
        ...
        <div className="flex flex-col gap-6 border-y border-[#1c1c1c] py-6 sm:flex-row sm:items-end sm:justify-between sm:gap-10">
          <div className="flex flex-wrap gap-x-14 gap-y-6">
            <MetaItem label="Discipline">{project.metaCategory}</MetaItem>
      ...
      {project.additionalImages.length > 0 && (
        <section className="mt-16 sm:mt-24">
          ...
          >
            {project.additionalImages.map((src, i) => (
              <motion.div key={src} variants={reveal}>
                <ClickableImage
                  src={src}
                  alt={`${project.title} — image ${i + 1} of ${project.additionalImages.length}`}
